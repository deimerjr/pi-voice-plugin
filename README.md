...
4. **Simulador TUI de Pi CLI & Terminal Theater [Gentleman-Sexy-Djr]**:
   - Ventana interactiva que emula la interfaz de terminal de Pi CLI.
   - Widget interactivo al pie con botones funcionales: alternancia de estado `Voice: ON/OFF`, botón `[STOP] Parar`, ajuste cíclico de volumen y simulación de dictado Whisper STT `[REC] Dictar`.
   - **Pi Terminal Theater (`theater.html`)**: Animación cinematográfica con la paleta de colores obsidian/rose del tema `Gentleman-Sexy-Djr`, osciloscopio reactivo en canvas y exportación directa de video WebM con audio (`MediaRecorder`).
...
👉 **`http://192.168.18.29:3000`** (Landing page principal)
👉 **`http://192.168.18.29:3000/theater.html`** (Animación cinematográfica de la terminal en vivo)
